  ...
  function installNetworkObserver() {
    ...
    if (typeof OrigXHR === 'function' && OrigXHR.prototype) {
      ...
      OrigXHR.prototype.open = function (method, url) {
        try { this.__tq = { method: method, url: url, headers: {} }; } catch (e) { /* ignore */ }
    ...
    try {
      switch (d.type) {
        case MSG.SET_CONFIG: {
          ...
          if (p.docIdOverrides && typeof p.docIdOverrides === 'object') {
            docIdOverrides = p.docIdOverrides;
          }
        ...
        case MSG.PLATFORM_BLOCK: {
          ...
          if (!ladder.length) {
            postTo(d.nonce, MSG.BLOCK_RESULT, {
              ticket, targetId, dryRun, ok: false, attempts: [],
              detail: 'the site\'s own block operation is not loaded on this page yet, and raw ' +
                      'request fallbacks are disabled. Open a profile\'s "..." menu once so the ' +
                      'module loads, or enable the raw fallback in settings.'
            });
            break;
          }
